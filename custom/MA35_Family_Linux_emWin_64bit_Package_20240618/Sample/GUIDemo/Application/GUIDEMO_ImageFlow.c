...
static const unsigned int _aSize[] = {
...
static int _GetImages(GUI_MEMDEV_Handle * phMem, GUI_MEMDEV_Handle * phMemNarrow, unsigned MaxItems) {
  ...
  for (i = 0; (i < GUI_COUNTOF(_apData)) && (i < MaxItems); i++) {
    ...
    *(phMem + i) = GUI_MEMDEV_CreateFixed32(0, 0, Info.XSize, Info.YSize);
    if (*(phMem + i) == 0) {
      return 1;
    }
  ...
  for (; i < MaxItems; i++) {
    *(phMem + i) = GUI_MEMDEV_CreateFixed32(0, 0, 150, 150);
    GUI_MEMDEV_Select(*(phMem + i));
    GUI_DrawGradientV(0, 0, 149, 149, GUI_MAKE_COLOR(0xCC500000), GUI_MAKE_COLOR(0x55500000));
  ...
  #endif
  for (i = 0; i < 6; i++) {
    *(phMemNarrow + i) = GUI_MEMDEV_CreateFixed32(0, 0, 60, 120);
    if (*(phMemNarrow + i) == 0) {
      return 1;
    }
  }
  ...
  for (i = 0; i < GUI_COUNTOF(_aParaL); i++) {
    ...
    if (i == 0) {
      for (j = 0; j < 6; j++) {
        ...
        GUI_SetBkColor(GUI_MAKE_COLOR(0xFFFFFF));
    ...
    yOff = 50;
    GUI_SetBkColor(GUI_MAKE_COLOR(0xFFFFFFFF));
