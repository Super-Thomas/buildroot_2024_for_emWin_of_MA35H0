...
typedef struct {
  U16 Flags;      // Flags that define the type of the font. Permitted values are explained in
                  // \ref{Font info flags}.
  U32 Baseline;    // Height of the baseline. The baseline is the line where most, but not all
                  // characters are 'placed on'. The lowest part of an 'A' is on the baseline.
  U32 LHeight;     // Height of a lower case character such as 'a' or 'x'.
  U32 CHeight;     // Height of an upper case character such as 'A' or 'X'.
} GUI_FONTINFO;
